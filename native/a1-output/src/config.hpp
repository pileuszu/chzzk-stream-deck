...
struct Config {
    ...
    bool calibrationVerified=false, videoEnabled=true;
...
Config config(const fs::path& path, bool requireVideo=false) {
    ...
    while(std::getline(f,line)) {
        ...
        if(k=="name") c.name=v;
        else if(k=="output_mode") { if(v!="audio_video"&&v!="audio_only") throw std::runtime_error("Invalid output mode"); c.videoEnabled=v=="audio_video"; }
    ...
    if(requireVideo) c.videoEnabled=true; // The OBS adapter always captures video, even with a legacy shared config.
    if(c.width<320||c.width>3840||c.width%2||c.height<180||c.height>2160||c.fps<15||c.fps>60||c.monitor<0||
       c.bufferMs<(c.videoEnabled?200:0)||c.bufferMs>2000||!std::isfinite(c.audioOffsetMs)||std::abs(c.audioOffsetMs)>c.bufferMs-(c.videoEnabled?100:0)||
