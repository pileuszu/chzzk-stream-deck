...
NDI 패널에서 시작 전에 해상도·FPS·화면 번호·버퍼·오디오 보정을 편집하고 저장할 수 있습니다. 소리만 모드는 `buffer_ms=0`과 `audio_offset_ms=0`으로 송신기의 추가 대기를 끌 수 있습니다. 장치·네트워크·수신 버퍼 지연은 남습니다. 소리만 버퍼는 0~2000 ms(보정 절댓값 ≤ 버퍼), 화면+소리와 로컬 OBS는 200~2000 ms(보정 절댓값 ≤ 버퍼−100 ms)를 사용합니다. 모드 전환 시 유효하지 않은 값은 저장·적용 전에 안내합니다.
NDI 패널의 **전송 대상**에서 **화면 + 소리 / 소리만**을 선택할 수 있습니다. `sender.ini`의 `output_mode=audio_only`는 화면 캡처와 영상 전송을 완전히 끄고 A1 믹스만 전송합니다. `audio_video` 또는 키가 없는 기존 설정은 화면+소리입니다. 중지 중에는 설정을 저장하고, 송출 중에는 **변경 적용**으로 잠시 재시작합니다. 실패하면 저장된 모드로 **송출 시작**을 다시 누르세요. 로컬 OBS 캡처에는 이 NDI 전용 선택을 적용하지 않습니다.
