...
class ChatOverlay {
    ...
    connectToServer() {
        ...
        this.eventSource.addEventListener('settings', event => {
            try { this.applySettings(JSON.parse(event.data)); }
            catch (error) { console.warn('채팅 설정 적용 실패:', error.message); }
        });
        this.eventSource.onmessage = event => {
            try { this.addMessage(JSON.parse(event.data)); }
    ...
    appendMessageText(element, text, extras) {
        ...
        for (const match of text.matchAll(/\{:(.*?):\}/g)) {
            ...
            let url;
            try {
                const candidate = new URL(emojis?.[match[1]]);
                if (candidate.protocol === 'https:' && /(^|\.)(pstatic\.net|naver\.net|naver\.com)$/.test(candidate.hostname)
                    && !candidate.username && !candidate.password) url = candidate.href;
            } catch {}
            if (url) {
                const image = document.createElement('img');
                image.className = 'emoticon'; image.alt = match[0]; image.src = url; image.referrerPolicy = 'no-referrer';
                image.addEventListener('error', () => image.replaceWith(document.createTextNode(match[0])), { once: true });
    ...
    addMessage(data) {
        ...
        this.messages.push({ element, data, timestamp: Date.now(), originalUsername: data.username });
        this.cleanupExcessMessages();
        this.armExpiry();
    }
