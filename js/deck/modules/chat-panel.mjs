...
<label class="chat-channel-field"><span>채널</span><input type="text" id="chat-channel-id" aria-label="CHZZK 채널 주소 또는 ID" placeholder="치지직 채널 주소 또는 ID" maxlength="1024" spellcheck="false" autocomplete="off"></label>
...
<button type="button" class="info-trigger" data-info-trigger aria-controls="chat-appearance" aria-haspopup="dialog" aria-expanded="false"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="9"/><path d="M12 11v6M12 7v.2"/></svg>글자·투명도</button>
</div>
...
<section id="chat-appearance" class="info-popover" popover="auto" role="dialog" tabindex="-1" aria-labelledby="chat-appearance-title" data-chat-display><header class="info-popover-heading"><h3 id="chat-appearance-title">글자·투명도</h3><button type="button" data-info-close aria-label="표시 설정 닫기"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m7 7 10 10M17 7 7 17"/></svg></button></header><div class="info-popover-body"><div class="field-grid"><label class="field">글자 크기 · px<input id="chat-font-size" type="number" min="12" max="36" required></label><label class="field">불투명도 · %<input id="chat-opacity" type="number" min="40" max="100" required></label></div><label><input id="chat-fade-mask" type="checkbox"> 위쪽 메시지 흐리게</label><p>미리보기에서 확인한 뒤 설정 저장을 누르세요.</p></div></section>
