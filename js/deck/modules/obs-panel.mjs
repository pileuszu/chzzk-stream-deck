...
<div class="capture-destination"><strong>OBS 장면 모음 · A1 Local</strong><small>소스: A1 Desktop + Audio</small></div>
...
<ol><li>OBS Studio 32.0.1 x64를 설치하고 한 번 실행한 뒤 종료합니다. 일반 설치 경로를 사용해 주세요.</li><li>Voicemeeter를 실행해 A1으로 마이크·기타·컴퓨터 소리를 듣도록 설정합니다.</li><li>소스를 클론했다면 저장소에서 <code>npm ci</code> → <code>npm run build:native</code> → <code>npm run app</code> 순서로 실행합니다. 네이티브 빌드에는 VS 2022 C++ Build Tools·Windows SDK·CMake 3.24 이상이 필요합니다.</li><li>자동 장면 등록에는 Python 3가 필요합니다. 준비 상태가 충족되면 아래 <b>OBS 소스 준비</b>를 누릅니다.</li></ol>
...
<ol><li>아래 <b>OBS 열고 전달 시작</b>을 누릅니다. OBS가 열려 있다면 <b>프로파일 → A1 Local</b>, <b>장면 모음 → A1 Local</b>을 선택합니다.</li><li><b>A1 Local Broadcast</b> 장면의 <b>A1 Desktop + Audio</b> 소스에서 화면과 오디오 미터가 움직이는지 확인합니다. 마이크·기타·컴퓨터 소리는 Voicemeeter A1에 섞인 소리로 들어옵니다.</li><li>OBS의 <b>설정 → 방송</b>에서 방송 서비스를 연결하고, <b>설정 → 출력</b>에서 인코더를 확인합니다. OBS 오른쪽 <b>방송 시작</b>을 누르면 송출됩니다. 먼저 <b>녹화 시작</b>으로 짧게 확인할 수도 있습니다.</li></ol>
