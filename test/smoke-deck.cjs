    ...
    window.webContents.once('did-finish-load', async () => {
        try {
            ...
            const screen = () => evaluate(`({
                ...
                dialogs: [...document.querySelectorAll('dialog, [role="dialog"]')].filter(element => element.getClientRects().length).length,
            ...
            await waitFor('window.app.uiManager.currentModule === "chat" && document.getElementById("chat-font-size").value !== ""');
            ...
            await waitFor('!window.app.chatBusy && window.app.settingsManager.getModuleSettings("chat").maxMessages === 6');
            await checkScreen('chat');
            ...
            await waitFor('window.app.uiManager.currentModule === "chat"');
            ...
            await waitFor('!window.app.chatBusy && window.app.settingsManager.getModuleSettings("chat").maxMessages === 5');
            ...
            const chatWorkspace = await evaluate(`(() => {
                ...
                const allInputsVisible = [...form.querySelectorAll('input:not([type=hidden]),select')].filter(input => !input.closest('[popover]')).every(input => input.getBoundingClientRect().height > 0);
                messages.value = '0';
