...
test('server shutdown joins chat, ends SSE and releases the HTTP listener', async t => {
    const Server = require('../server');
    const server = new Server();
    const child = fork(path.join(__dirname, 'fixtures/managed-child.cjs'), [], { stdio: ['ignore', 'ignore', 'ignore', 'ipc'], windowsHide: true });
    t.after(() => { if (child.exitCode == null && child.signalCode == null) child.kill(); });
    await once(child, 'message');
    server.processes.chat = child; server.status.chat.active = true;
    let ended = false; server.sseConnections.add({ end() { ended = true; } });
    ...
    assert.equal(child.exitCode, 0); assert.equal(ended, true);
    assert.equal(server.serverInstance.listening, false); assert.equal(server.processes.chat, null);
    await assert.rejects(fetch('http://127.0.0.1:' + address.port));
    await assert.rejects(server.startChatModule('fixture'), /종료/);
});
