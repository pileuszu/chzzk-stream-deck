...
test('release refuses incomplete modules, private files and customized sender settings', t => {
    ...
    f.write(plugin);
    const settings = f.output + 'win-unpacked/resources/app/.data/settings.json';
    f.write(settings, 'private');
    assert.throws(() => prepareArtifacts(f.root), /Private\/build/);
    fs.rmSync(path.dirname(path.join(f.root, settings)), { recursive: true });
