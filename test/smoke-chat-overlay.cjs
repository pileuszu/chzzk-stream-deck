...
app.whenReady().then(async () => {
    ...
    const server = new Server({ settingsPath, clientFactory: id => { const client = new (require('node:events').EventEmitter)(); client.channelId = id; client.state = 'idle'; client.getStatus = () => ({ state: client.state, active: client.state !== 'idle', connected: client.state === 'connected' }); client.start = async () => { client.state = 'connected'; client.emit('status'); }; client.disconnect = () => { client.state = 'idle'; }; return client; } });
    ...
    try {
        ...
        server.addMessage({ username:'긴닉네임시청자', message:'저장 전부터 표시된 채팅' });
        ...
        for (const theme of ['simple-purple','maplestory','unicorn-overlord']) {
            const savedTheme = server.store.get().theme;
            await set('theme-select', theme);
            await delay(100);
            assert.equal(server.store.get().theme, savedTheme, 'draft must not reach OBS');
            ...
            assert.equal(server.store.get().theme, theme);
        ...
        for (let i=0;i<3;i++) server.addMessage({username:'긴닉네임시청자',message:'수신 확인 '+i});
        ...
        const update=server.store.update.bind(server.store);
        server.store.update=()=>{throw new Error('test disk unavailable');};
        await set('theme-select','maplestory');
        await evaluate('document.getElementById("save-chat-settings").click()');
        await wait('!window.app.chatBusy');
        assert.equal(server.store.get().theme,'unicorn-overlord');
        ...
        await evaluate('document.getElementById("chat-connect").click()');
        await wait('window.app.chatModule.isActive && !window.app.chatBusy');
        assert.equal(server.store.get().theme,'maplestory');
        assert.equal(server.store.get().channelId,'a'.repeat(32));
        await evaluate("document.querySelector('[aria-controls=\"chat-appearance\"]').click()");
        await set('font-size',22); await set('opacity',85);
        await evaluate("document.getElementById('chat-fade-mask').click()");
        await evaluate("document.querySelector('#chat-appearance [data-info-close]').click();document.getElementById('save-chat-settings').click()");
        await wait('!window.app.chatBusy');
        assert.equal(server.store.get().fontSize,22); assert.equal(server.store.get().opacity,85);
        assert.equal(server.store.get().fadeMask,true);
        await wait('window.chatOverlay.settings.fontSize === 22 && window.chatOverlay.settings.opacity === 85',live);
        ...
        server.addMessage({username:'<script>',message:'<img src=x onerror=alert(1)>'});
