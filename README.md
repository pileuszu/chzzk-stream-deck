...
- 로컬 캡처는 화면·오디오·싱크 설정을 편집하고 하단에서 저장·적용·시작·중지를 조작합니다. NDI 패널은 송출 전 해상도·FPS·버퍼·오디오 보정과 전송 대상을 설정하고 실제 송출 상태를 표시합니다. NDI와 로컬 캡처의 A1 사용은 동시에 실행되지 않습니다.
...
- 트레이 우클릭 → **종료**는 진행 중인 작업을 기다린 뒤 로컬 A1 캡처, NDI 송신기, 채팅 연결를 정리하고 서버·트레이·앱을 종료합니다. 일부 모듈이 종료되지 않으면 오류를 표시하고 재시도할 수 있도록 앱을 유지합니다.
...
채널 ID 또는 치지직 채널·라이브 URL을 입력할 수 있습니다. 인증이 완료되어야 연결됨으로 표시하며, 연결이 끊기면 자동 재연결합니다. 글자 크기·불투명도·상단 흐림은 채팅 패널의 **글자·투명도**에서 조정합니다.

SSE는 snapshot·chat·settings·status·clear 이벤트를 보냅니다. 다시 연결한 오버레이는 저장된 설정과 메시지 시각을 복원하며, 유지 시간이 지난 메시지는 되살리지 않습니다. 서버는 공개 UI 파일만 제공하고 다른 출처에서의 API 쓰기를 거절합니다.
...
- `GET /api/chat/settings`, `POST /api/chat/settings` - Persist validated settings shared by the preview and OBS
- `POST /api/chat/clear` - Clear retained messages and connected overlays
...
### System Requirements
- **Node.js**: 22.12.0 or higher (Node.js 24 recommended)
...
`npm run check` checks JavaScript and module syntax. `npm test` runs chat, settings, renderer, lifecycle and native-output control tests on Windows, macOS and Linux. Windows UI smoke checks use isolated Electron profiles: `npx electron test/smoke-deck.cjs`, `npx electron test/smoke-local.cjs`, `npx electron test/smoke-ndi-panel.cjs`, `npm run test:chat-overlay`. Native A1 capture and NDI output are Windows-only.
