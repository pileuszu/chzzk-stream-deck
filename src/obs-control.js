...
function request(action, configPath, { endpoint = PIPE, timeout = 5000 } = {}) {
    ...
    return new Promise((resolve, reject) => {
        ...
        socket.on('connect', () => socket.write(JSON.stringify({ version: 2, id, action,
        ...
        socket.on('data', chunk => {
            ...
            if (reply.version !== 2 || !reply.ok || (action !== 'status' && reply.request_id !== id)) return finish(new Error(reply.error || 'OBS 연결 응답이 올바르지 않습니다.'));
